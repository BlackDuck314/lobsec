...
function toLlmRequest(req, body) {
    const headers = {};
...
function loadTlsOptions() {
    try {
        ...
        console.error(`[proxy] Failed to load TLS certificates from ${TLS_DIR}: ${err.message}`);
    ...
    async function handleRequest(req, res) {
        ...
        try {
            ...
            if (route.provider === "anthropic") {
                ...
                if (version)
                    outHeaders["anthropic-version"] = version;
            }
            else {
            ...
            const latencyMs = Date.now() - startTime;
            onError?.(err, `forwarding to ${route.targetUrl}`);
            if (onAudit) {
                onAudit({
                    provider: route.provider,
                    model: route.model ?? "unknown",
...
// ── CLI entry point ─────────────────────────────────────────────────────────
export function startProxyFromEnv() {
