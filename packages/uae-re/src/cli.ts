...
import { runWeeklyDigest } from "./analytics/weekly-digest.js";
import { runMonthlyReport } from "./analytics/monthly-report.js";
import { checkAndSendAlerts } from "./analytics/alerts.js";
...
Commands:
  ...
  weekly-digest           Generate and send weekly intelligence digest via Telegram
  monthly-report          Generate and send monthly intelligence report via Telegram
  check-alerts            Check for anomalies, failures, distress and send Telegram alerts
...
async function runAll(): Promise<void> {
  ...
  // Exit with failure only if ALL collectors failed (partial success = exit 0)
  if (result.successCount === 0 && result.failureCount > 0) {
    process.exit(1);
  }
  if (normalizationErrors > 0 && result.successCount === 0) {
    process.exit(1);
  }
}
...
async function runWeeklyDigestCmd(): Promise<void> {
  const db = initDatabase(dataDir);
  try {
    const sent = await runWeeklyDigest(db);
...
async function runMonthlyReportCmd(): Promise<void> {
  const db = initDatabase(dataDir);
  try {
    const sent = await runMonthlyReport(db);
    console.log(JSON.stringify({ sent }));
    if (!sent) process.exit(1);
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
...
/**
 * Run proactive alert check and send Telegram alerts with deduplication
 */
async function runCheckAlerts(): Promise<void> {
  const db = initDatabase(dataDir);
  try {
    const result = await checkAndSendAlerts(db);
    console.log(JSON.stringify(result));
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
...
async function main(): Promise<void> {
  try {
    ...
    switch (command) {
      ...
      case "weekly-digest":
        await runWeeklyDigestCmd();
        break;

      case "monthly-report":
        await runMonthlyReportCmd();
        break;
