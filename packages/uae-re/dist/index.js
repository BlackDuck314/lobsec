...
export { insertRawSource, insertNormalized, queryNormalized, insertCollectionLog, getLatestCollection, insertArea, getCanonicalName, getAllAreas, addAreaAlias, deleteNormalizedRange, } from "./db/queries.js";
// Cache layer
...
// Area mapping
export { initAreaTable, getCanonicalArea, getAreaAliases, addDiscoveredArea, getAllAreas as getAllAreasFromMapping, } from "./areas/mapping.js";
...
export { SOURCE_MODULE_MAP, } from "./normalization/types.js";
...
const Type = {
    Object: (props, opts) => {
        const required = Object.keys(props).filter((k) => !(props[k] && typeof props[k] === "object" && OPTIONAL_MARKER in props[k]));
        // Strip optional markers from properties before emitting schema
        const cleanProps = {};
        for (const [k, v] of Object.entries(props)) {
            if (v && typeof v === "object" && OPTIONAL_MARKER in v) {
                const { [OPTIONAL_MARKER]: _, ...rest } = v;
                cleanProps[k] = rest;
            }
            else {
                cleanProps[k] = v;
    ...
    Optional: (schema) => ({ ...schema, [OPTIONAL_MARKER]: true }),
};
// Plugin entry point
export default {
    id: "lobsec-uae-re",
    register(api) {
        ...
        try {
            ...
            function resolveAreaOrError(input) {
            ...
            api.registerTool({
                ...
                parameters: Type.Object({
                    ...
                    property_type: Type.Optional(Type.String({
                ...
                execute: async (_id, params) => {
                    try {
                        ...
                        const cached = cache.get("area_signal", cacheKey);
                        if (cached)
                            return textResult(resolved.prefix + cached.formattedText);
                        const result = queryAreaSignal(db, resolved.result.canonical);
                        if (!result)
                            return textResult(`No area signal data available for "${resolved.result.canonical}".`);
                ...
                execute: async (_id, params) => {
                    try {
                        if (params.area) {
                            ...
                            const cached = cache.get("distress", cacheKey);
                            if (cached)
                                return textResult(resolved.prefix + cached.formattedText);
                            const result = queryDistress(db, resolved.result.canonical);
                            if (!result)
                                return textResult(`No distress data available for "${resolved.result.canonical}".`);
                            ...
                            const cached = cache.get("distress_topN", cacheKey);
                            if (cached)
                 ...
                 ORDER BY score DESC LIMIT 5`)
                            ...
                            const lines = [
                                ...
                                "",
                                ...rows.map((r, i) => `${i + 1}. ${r.area}: ${r.score.toFixed(2)}`),
                            ];
                ...
                execute: async (_id, params) => {
                    try {
                        ...
                        if ("error" in resolved)
                            return textResult(resolved.error);
                        const cacheKey = { area: resolved.result.canonical };
                        const cached = cache.get("rental_intel", cacheKey);
                        if (cached)
                            return textResult(resolved.prefix + cached.formattedText);
                        const result = queryRentalIntel(db, resolved.result.canonical);
                        if (!result)
                            return textResult(`No rental intelligence data available for "${resolved.result.canonical}".`);
            ...
            api.registerTool({
                ...
                execute: async (_id, params) => {
                    try {
                        ...
                        const cached = cache.get("supply_pipeline", cacheKey);
                        if (cached)
                            return textResult(prefix + cached.formattedText);
            ...
            api.registerTool({
                ...
                execute: async (_id, _params) => {
                    try {
                        const cacheKey = {};
                        const cached = cache.get("expat_flow", cacheKey);
                        if (cached)
                            return textResult(cached.formattedText);
            ...
            api.registerTool({
                ...
                execute: async (_id, _params) => {
                    try {
                        const cacheKey = {};
                        const cached = cache.get("macro_health", cacheKey);
                        if (cached)
                            return textResult(cached.formattedText);
            ...
            api.registerTool({
                ...
                parameters: Type.Object({
                    ...
                    property_type: Type.Optional(Type.String({ description: "apartment | villa | townhouse" })),
                }),
                execute: async (_id, params) => {
                    try {
                        const area = String(params.area ?? "");
                        ...
                        const cached = cache.get("arbitrage", cacheKey);
                        if (cached)
                            return textResult(resolved.prefix + cached.formattedText);
                        const result = queryArbitrage(db, resolved.result.canonical);
                        if (!result)
                            return textResult(`No arbitrage data available for "${resolved.result.canonical}".`);
            ...
            api.registerTool({
                ...
                parameters: Type.Object({
                    income_bracket: Type.Optional(Type.String({
                ...
                execute: async (_id, params) => {
                    try {
                        ...
                        const cached = cache.get("salary_rent", cacheKey);
                        if (cached)
                            return textResult(cached.formattedText);
            ...
            function nextScheduledRun(frequency) {
                const now = new Date();
                ...
                switch (frequency) {
                    ...
                    case "quarterly": {
                        ...
                        let nextQMonth = quarterMonths.find((m) => m > currentMonth || (m === currentMonth && currentDay < 15));
                        if (nextQMonth === undefined)
                            nextQMonth = 0; // wrap to Jan next year
                        const nextYear = nextQMonth <= currentMonth && currentDay >= 15
            ...
            api.registerTool({
                ...
                description: "Show collection status for all UAE RE data sources. Returns last run times, staleness flags, normalized row counts, and next scheduled run.",
                parameters: Type.Object({}),
                execute: async (_id, _params) => {
                    try {
                        const collectors = registry.getAll();
                        if (collectors.length === 0) {
                            return textResult("No collectors registered. This should not happen.");
                        }
                        ...
                        for (const info of collectors) {
                            ...
                            const rowCountRow = db
                                .prepare("SELECT COUNT(*) as cnt FROM normalized_monthly WHERE source = ?")
                                .get(info.metadata.source);
                            const normalizedRows = rowCountRow?.cnt ?? 0;
                            const staleMarker = isStale ? " [STALE]" : "";
                            lines.push(`Source: ${info.metadata.source}${staleMarker}`);
                            lines.push(`  Frequency: ${info.metadata.frequency} | Priority: ${info.metadata.priority}`);
                            if (lastRun) {
            ...
            api.registerTool({
                name: "uae_raw_data",
                label: "UAE Raw Data",
                description: "Returns normalized data rows for a source in CSV format. Specify date range or get last 12 months.",
                parameters: Type.Object({
                    source: Type.String({
                        description: "Data source name (e.g. dld-sales, ejari-rentals, bayut-listings)",
                    }),
                    start_date: Type.Optional(Type.String({ description: "Start date YYYY-MM-DD (default: 12 months ago)" })),
                    ...
                    try {
                        ...
                        if (!collectorInfo) {
                            const allSources = registry
                                ...
                                .join(", ");
                            return textResult(`Unknown source: "${source}". Valid sources: ${allSources}`);
                        }
                        const now = new Date();
                        const defaultStart = new Date(now);
                        defaultStart.setFullYear(defaultStart.getFullYear() - 1);
                        const startDate = String(params.start_date ?? defaultStart.toISOString().slice(0, 10));
                        const endDate = String(params.end_date ?? now.toISOString().slice(0, 10));
                        const rows = db
                            .prepare(`SELECT source, measurement_date, metric_name, value
               ...
               ORDER BY measurement_date, metric_name`)
                        ...
                        if (rows.length === 0) {
                            return textResult(`No data for "${source}" between ${startDate} and ${endDate}.`);
                        }
                        // Build CSV from normalized rows
                        const csvLines = ["source,measurement_date,metric_name,value"];
                        for (const row of rows) {
                            csvLines.push(`${row.source},${row.measurement_date},${row.metric_name},${row.value}`);
                        }
                        const csv = csvLines.join("\n");
                        const truncated = csv.length > 4000
            ...
            api.registerTool({
                ...
                parameters: Type.Object({
                    source: Type.Optional(Type.String({
                        description: "Source to collect (e.g. dld-sales). Omit to trigger all sources.",
                    })),
                }),
                execute: async (_id, params) => {
                    try {
                        ...
                        if (source) {
                            ...
                            setImmediate(() => {
                                registry.runOne(source).catch((err) => {
                                    const msg = err instanceof Error ? err.message : String(err);
                                    log.error(`[lobsec-uae-re] trigger_collection error for ${source}: ${msg}`);
                                });
                            });
                            return textResult(`Collection triggered for: ${source}. Use uae_collection_status() to monitor progress.`);
                        }
                        else {
                            setImmediate(() => {
                                registry.runAll().catch((err) => {
                                    const msg = err instanceof Error ? err.message : String(err);
                                    log.error(`[lobsec-uae-re] trigger_collection error for all: ${msg}`);
                                });
                            });
                            return textResult(`Collection triggered for: all sources. Use uae_collection_status() to monitor progress.`);
                        }
                    }
                    catch (err) {
                        return textResult(`Error: ${err instanceof Error ? err.message : String(err)}`);
            ...
            api.registerTool({
                ...
                parameters: Type.Object({
                    signal: Type.String({
                        description: "Signal series as source|metric, e.g. 'bayut-listings|avg_asking_price'",
                    }),
                    target: Type.String({
                        description: "Target series as source|metric, e.g. 'dld-sales|avg_price_per_sqft'",
                    }),
                    max_lag: Type.Optional(Type.Number({ description: "Maximum lag in months (default: 6)" })),
                }),
                execute: async (_id, params) => {
                    try {
                        ...
                        if (!signal.includes("|"))
                            return textResult(`signal must be 'source|metric', e.g. 'bayut-listings|avg_asking_price'`);
                        if (!target.includes("|"))
                            return textResult(`target must be 'source|metric', e.g. 'dld-sales|avg_price_per_sqft'`);
                        ...
                        if (!result.success) {
                            return textResult(`Granger test failed: ${result.error ?? "unknown error"}`);
                        }
                        const data = result.data;
                        if (data?.error) {
                            return textResult(`Granger test failed: ${data.error}`);
                        }
                        return textResult(data?.output ?? JSON.stringify(data));
                    }
                    catch (err) {
                        return textResult(`Error: ${err instanceof Error ? err.message : String(err)}`);
                    }
            ...
            api.registerTool({
                ...
                parameters: Type.Object({
                    ...
                    max_lag: Type.Optional(Type.Number({ description: "Maximum lag in months (default: 12)" })),
                }),
                execute: async (_id, params) => {
                    try {
                        const signal = String(params.signal ?? "");
                        ...
                        if (!result.success) {
                            return textResult(`Correlation analysis failed: ${result.error ?? "unknown error"}`);
                        }
                        const data = result.data;
                        if (data?.error) {
            ...
            log.info("[lobsec-uae-re] registered 34 collectors + 13 tools");
        }
        catch (error) {
            const msg = error instanceof Error ? error.message : String(error);
