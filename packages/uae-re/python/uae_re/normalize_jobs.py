...
def adapt_scraper_format(raw_data) -> dict:
    """Convert Ninja Scraper list-of-pages format to normalizer dict format.

    Scraper produces: [{url, cards: [{job_title, company, ...}], card_count, total_count}, ...]
    Normalizer expects: {listings: [{job_title, salary_min, salary_max, ...}], total_count: int}
    """
    ...
    for page in raw_data:
        ...
        # total_count may be string like "1,000+", "12.4K jobs found", or int
        tc = page.get("total_count")
        if tc is not None:
            if isinstance(tc, str):
                # Extract leading numeric part: "12.4K jobs found" -> "12.4K", "1,000+" -> "1,000"
                m = re.match(r'[\d,]+\.?\d*\s*[KkMm]?', tc)
                if m:
                    ...
                    if num_str[-1] in ('K', 'k'):
                        ...
                        num_str = num_str[:-1]
                    try:
                        tc = int(float(num_str.replace(",", "")) * multiplier)
                    except ValueError:
                        tc = 0
                else:
                    tc = 0
    ...
    # Map card fields to normalizer expectations
...
def main():
    ...
    try:
        ...
        # Adapt scraper list-of-pages format to normalizer dict format
