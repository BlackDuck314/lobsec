"""
Commodity Price Collector via Yahoo Finance v8 API (COMM-01, COMM-02)

Fetches monthly OHLCV data for Brent Crude Oil (BZ=F) and Gold (GC=F)
...
def collect_commodities(output_dir: str) -> dict:
    """Fetch monthly OHLCV data for commodity futures.
    ...
    out_path = commodities_dir / f"{today}.json"
    ...
    for idx, (symbol, info) in enumerate(COMMODITIES.items()):
        print(f"Fetching {symbol} ({info['name']}) from Yahoo Finance...", file=sys.stderr)
        ...
        if data is None:
            ...
            commodities_data[symbol] = {"timestamps": [], "ohlcv": {}}
            continue
        ...
        try:
            ...
            print(f"  WARNING: Failed to parse {symbol}: {e}", file=sys.stderr)
            commodities_data[symbol] = {"timestamps": [], "ohlcv": {}}

        # Rate limiting: pause between requests
        if idx < len(COMMODITIES) - 1:
            time.sleep(1)
...
def main() -> None:
    ...
    try:
        ...
        result = collect_commodities(output_dir)
